...
export  async function editMaindishes(item,id,maindishid){ 
    let maindish = {
        name: item.name,
        desc: item.desc,
        foodEnable:item.foodEnable,
    ...
    return await firebase.db.collection('Restaurants').doc(id).collection('MainDishes').doc(maindishid).update(maindish)
}
